...
class Controller: ObservableObject, LoggerDelegate, DFUServiceDelegate, DFUProgressDelegate {
    ...
    // Local STT
    private let _speechToText = SpeechToTextConverter()
    ...
    private func onVoiceReceived(voiceSample: AVAudioPCMBuffer) {
        ...
        _m4aWriter.write(buffer: voiceSample) { [weak self] fileURL, fileData in
            guard let self = self, let unwrappedFileURL = fileURL, let data = fileData else {
                self?.printErrorToChat("Unable to process audio!", as: .user)
                return
            }
            // Apple Speech to Text
            self.printTypingIndicatorToChat(as: .transcriber)
            _speechToText.convert(fileURL: unwrappedFileURL) { result in
                switch result {
                case .success(let transcription):
                    // Handle successful transcription here
